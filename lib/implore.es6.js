...
const hashRequestObject = new Map();
const xmlHttpRequests = new Map();
...
export default function implore (request) {
	let url = getURLFromRequest(request);

	if (xmlHttpRequests.has(url) && cacheableMethods.has(request.method)) {
		return xmlHttpRequests.get(url);
	}
	...
	xmlHttpRequests.set(url, promise);
	promise.then(clearCache(url), clearCache(url));
 ...
 * @return {function} call the function to clear the cache
 */
